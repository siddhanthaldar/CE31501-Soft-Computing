...
def actual_val(population, limits):  # limits represents range of the numbers = [min,max]
	act_val = np.zeros((population, 2))  # (x1,x2)
	diff = limits[1] - limits[0]
	str_len = population.shape[1]/2
...
def Fx(act_val):
	F = np.zeros(act_val.shape[0])

	for i in range(act_val.shape[0]):
		f = func(act_val[i][0],act_val[i][1])
		F[i] = 1.0/(1.0+f)
...
def E(C,D):
	E_arr = np.zeros(C.shape[0])
	for i in range(C.shape[0]):
		for j in range(C.shape[0]-1):		
			if D[i]>=C[j] and D[i]<C[j+1]:
...
def mating_pool_update(strings, E):
	mating_pool = np.zeros(strings.shape)
	for i in range(E.shape[0]):
		mating_pool[i] = strings[i]
	return mating_pool

def crossover(mating_pool, crossover_prob):
	crossover_mating_pool = mating_pool
	num_crossover = mating_pool.shape[0]/2
	crossover_point = np.zeros(num_crossover)
	for i in range(num_crossover):
		crossover_point[i] = random.randint(0,mating_pool.shape[1]/2-2)

	# array to keep track of elements crossed over
	crossed = np.zeros(mating_pool.shape[0])
	...
	for i in range(num_crossover):
		...
		j = random.randint(0,mating_pool.shape[0]-1)
		k = random.randint(0,mating_pool.shape[0]-1)
		...
		crossover_mating_pool[j][0:cp] = mating_pool[k][0:cp]
		crossover_mating_pool[j][10:10+cp] = mating_pool[k][10:10+cp]
		crossover_mating_pool[k][0:cp] = mating_pool[j][0:cp]
		crossover_mating_pool[k][10:10+cp] = mating_pool[j][10:10+cp]
...
def mutation(mating_pool, mutation_prob):
	mutation_index = np.zeros(mating_pool.shape[0])
	for i in range(mating_pool.shape[0]):
		mutation_index[i] = random.randint(0, mating_pool.shape[1]/2-1)

	for i in range(mating_pool.shape[0]):
		if(random.random()<1-mutation_prob):
			continue
		mating_pool[i][int(mutation_index[i])] = 1 - mating_pool[i][int(mutation_index[i])]

	return mutation

def iteration(mating_pool, limits, crossover_prob, mutation_prob):
	print(mating_pool.shape[0])
